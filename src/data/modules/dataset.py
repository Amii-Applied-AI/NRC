...
Description: Stores the classes defining the different type of datasets used during experiments.
"""
...
class SeqDataset(Dataset):
    """
    Datasets dedicated to train and evaluate a contrast model with gene sequences.
    """
    def __init__(self,
                 seq_a: Tensor,
                 seq_b: Tensor,
        ...
        Args:
            seq_a (Tensor): encoded gene sequences (N, C, 3000) or 
                            tokenized gene sequences (N, 6, 512).
    ...
    @property
    def labels_mean(self) -> float:
        """
        Returns the mean of the labels.

        Returns:
            float: mean of the labels.
        """
        return self.__labels.mean().item()

    @property
    def labels_std(self) -> float:
        """
    ...
    def from_path(cls,
        ...
        Args:
            ...
            tokens (bool, optional): if True, indicates that the data 
                                     ...
                                     Default to False.
            proseq (bool, optional): if True, targets will be taken from PRO-seq data.
                                     Otherwise, targets will be extracted from QuantSeq data.
                                     Default to False. 
            remove_zeros (bool, optional): if True, pairs of orthologs with a target of zero 
                                           are removed from the dataset.
                                           Default to False.
            scaler (Scaler, optional): if given, the scaling method is applied to the targets.
                                       Default to None.
            regression (bool, optional): if False, targets are changed to binary classes
                                         by checking if they are >= 0. 
                                         Default to False
        """
        # Extract data
        if proseq:
            with open(path, 'rb') as file:
                _, x, _, y, species, _ = load(file)
        else:
            with open(path, 'rb') as file:
                _, x, y, _, species, _ = load(file)

        # Filter for null targets
        if remove_zeros:
            non_null = y != 0
            x, y, species = x[non_null], y[non_null], species[non_null]

        # Filter data according to the given specie
        if specie is not None:
            if specie not in Maps.PAIR2ID.values():
                raise ValueError(f'specie must be one of {Maps.PAIR2ID.values()}')

            idx_to_keep = (species == specie).nonzero().squeeze()
            x, y, species = x[idx_to_keep], y[idx_to_keep], species[idx_to_keep]

        # If data was filtered
        if specie is not None:

            # Balance the remaining data
            x, y, _ = cls.balance_data(x, y)

        # If current data holds tokenized sequences (N, 2, 6, 512)
        if tokens:
            seq_a = x[:, 0, :, :].squeeze()
            seq_b = x[:, 1, :, :].squeeze()
    ...
    def __train_getter(self, idx: list[int]) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        ...
        Returns:
            tuple[Tensor, Tensor, Tensor, Tensor]: seq_a (N, *), seq_b (N, *),
                                                   labels (N,), species (N,)
        """
        ...
        # According to the success (or failure) associated to Bernoulli samples,
    ...
    def flip_reg_labels(labels: Tensor) -> Tensor:
        ...
        Returns:
            Tensor: opposite of regression labels (N,).
        """
        return -labels
    ...
    def get_token_reverse_complement(seq: Tensor) -> Tensor:
        ...
        Args:
            seq (Tensor): tokenized gene sequences (N, 6, 512).

        Returns:
            Tensor: reverse complement of gene sequences (N, 6, 512).
        """
        # Load the reverse complement map
        id_to_reverse_comp_id = SeqDataset.load_reverse_complement_map()
    ...
    def find_idx_to_flip(binary_class_labels: Tensor) -> Tensor:
        ...
        Args:
            binary_class_labels (Tensor): tensor with binary values (N,)
        ...
        ones_count = binary_class_labels.sum().item()
        zeros_count = binary_class_labels.shape[0] - ones_count
